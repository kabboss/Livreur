...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        console.log(`📈 Efficacité calculée: ${enrichedData.qualityMetrics.overallEfficiency}%`);
...
function analyzePerformance(productionData) {
    ...
    if (quantity < 50) {
        ...
        analysis.recommendations.push("Optimiser les processus pour petites productions");
    } else if (quantity > 200) {
        analysis.category = "large-batch";
        analysis.recommendations.push("Surveiller la température de manière continue");
        analysis.riskFactors.push("Risque de variation de température sur gros volume");
    }

    if (productionData.incubationHours < 6) {
        analysis.riskFactors.push("Temps d'incubation court - surveiller la fermentation");
    } else if (productionData.incubationHours > 8) {
        analysis.recommendations.push("Surveiller l'acidité - temps d'incubation élevé");
    }
...
async function cleanupOldProductions(collection) {
    try {
        ...
        if (result.deletedCount > 0) {
            console.log(`🧹 Nettoyage: ${result.deletedCount} anciennes productions supprimées`);
        }
    } catch (cleanupError) {
