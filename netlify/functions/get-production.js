...
exports.handler = async (event, context) => {
  ...
  try {
      ...
      collection.aggregate([
        { $match: query },
        {
          $group: {
            ...
            totalRevenue: { $sum: { $toDouble: '$costAnalysis.expectedRevenue' } },
            productionsByStatus: {
              $push: '$status'
            }
          }
    ...
    const enrichedProductions = productions.map(production => {
      // Récupérer les photos complètes si demandé spécifiquement
      if (params.includePhotos === 'true' && production._id) {
        // Dans ce cas, faire une requête séparée pour les photos
      ...
      return {
        ...
        calculatedMetrics: {
          ...
          costPerLiter: production.costAnalysis ? 
            (parseFloat(production.costAnalysis.totalCost) / parseFloat(production.milkQuantity)).toFixed(2) + '€' : 'N/A'
        }
      };
    });
    ...
    const response = {
      ...
      aggregatedStats: {
        ...
        estimatedRevenue: Math.round(stats.totalRevenue || 0),
        statusDistribution
      },
