    ...
    try {
        ...
        if (existingDelivery) {
            return {
                ...
                body: JSON.stringify({ success: false, message: 'Cette commande a déjà été acceptée par un livreur' }),
            };
        }
        ...
        // Mettre à jour le statut dans la collection des commandes
        ...
        return {
            statusCode: 200,
            headers: { 'Access-Control-Allow-Origin': '*' },
            body: JSON.stringify({ success: true, message: 'Livraison acceptée avec succès' }),
        };
    } catch (error) {
