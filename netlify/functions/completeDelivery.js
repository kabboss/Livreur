...
exports.handler = async function(event, context) {
    ...
    try {
        ...
        // Enregistrer dans les livraisons effectuées
        await db.collection('LivraisonsEffectuees').insertOne({
            ...
            positions: await db.collection('cour_livraison')
                .find({ codeID: data.codeID })
                .toArray()
        });

        // Supprimer des collections temporaires
        await db.collection('cour_expedition').deleteOne({ codeID: data.codeID });
        await db.collection('cour_livraison').deleteMany({ codeID: data.codeID });
