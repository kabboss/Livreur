...
exports.handler = async (event) => {
  ...
  // ✅ Restriction sur POST uniquement
  ...
  try {
    ...
    if (!orderId || !newStatus) {
      return {
        statusCode: 400,
        headers: COMMON_HEADERS,
        body: JSON.stringify({ error: "orderId et newStatus sont requis." })
      };
    }
    ...
    if (result.modifiedCount === 0) {
      return {
        statusCode: 404,
        headers: COMMON_HEADERS,
        body: JSON.stringify({ error: "Commande non trouvée ou statut déjà à jour." })
      };
    }
